...
class User extends Table
{
    ...
    public  function register($firstname, $lastname, $email, $password)
    {
        return $this->exec(
            false,
            "INSERT INTO $this->table_name (fullname, email, password_hash) VALUES (?,?,?);",
            "sss",
            "$firstname $lastname",
            $email,
    ...
    public function changeProfile($picture)
    {
        if (gettype($this->id) === "integer" && isset($this->id)) {
            return $this->exec(false, "UPDATE user SET profile_pic = ? WHERE id = ?;", "si", $picture, $this->id);
        }
    }

    public function getPicture($id)
    {
        return $this->exec(false, "SELECT profile_pic FROM user WHERE id = ?;", "i", $id);
    }
}
