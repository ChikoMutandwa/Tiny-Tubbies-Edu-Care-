...
function updateUserProfile()
{
    ...
    if (isset($_POST['update_user_profile'])) {
        list("fullname" => $fullname, "email" => $email, "password" => $password) = $_POST;
        $newPassword = password_hash($password, PASSWORD_DEFAULT);

        $users = User::query("UPDATE user SET email='$email', fullname='$fullname', password_hash='$newPassword' WHERE id='$user->id'", "User");
...
function displayFiles()
{
    ...
    foreach ((new Files)->getAll() as $_file) {
        ...
        // $files = Files::query("SELECT * FROM file_uploads WHERE file_name AND `file_path`", "FIles");
...
{
    $user = $_SESSION['user'];
    $user_picture = User::query("SELECT profile_pic FROM user WHERE id = $user->id");

    foreach ($user_picture as $get_picture) {
        $get = <<<display
            <img class="image" src="$get_picture->profile_pic" alt="$get_picture->profile_pic">
        display;
        echo $get;
    }
}
